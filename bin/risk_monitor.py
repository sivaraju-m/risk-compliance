...
class RiskMonitoringService:
    ...
    def _load_expanded_universe(self) -> Dict[str, List[str]]:
        ...
        try:
            ...
            universe_paths = [
                Path(__file__).parent.parent.parent.parent / "ai-trading-machine" / "config" / "expanded_universe.json",
            ...
            for universe_path in universe_paths:
                if universe_path.exists():
                    ...
                    self.logger.info(f"Loaded expanded universe with {universe_data['metadata']['total_tickers']} tickers")
            ...
            return {
                "sample": ["RELIANCE", "TCS", "INFY", "HDFCBANK", "ICICIBANK"]
            }
    ...
    async def start(self):
        ...
        self.logger.info(f"Monitoring {sum(len(tickers) for tickers in self.universe.values())} symbols across {len(self.universe)} categories")
        ...
        try:
            while self.running:
                ...
                await asyncio.sleep(self.config.get('monitoring_interval', 60))  # Check every minute
    ...
    async def _monitor_risks(self):
        ...
        try:
            ...
            # Check risk limits
            risk_violations = []
            for alert in self.risk_monitor.check_portfolio_risk(positions, returns_data):
            ...
            if compliance_violations:
                self.logger.warning(f"Compliance violations detected: {len(compliance_violations)}")
                await self._handle_compliance_violations(compliance_violations)
            
            # Update monitoring dashboard
            await self._update_monitoring_dashboard(risk_metrics, risk_violations, compliance_violations)
    ...
    async def _get_returns_data(self, symbols: List[str]) -> Dict[str, pd.Series]:
        ...
        try:
            ...
            for symbol in symbols:
                ...
                returns = np.random.normal(0.001, 0.02, len(dates))  # Daily returns ~0.1% mean, 2% std
    ...
    async def _get_portfolio_data(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                try:
                    async with session.get("http://localhost:8002/api/portfolio", timeout=5) as response:
    ...
    async def _get_portfolio_from_files(self) -> Dict[str, Any]:
        ...
        try:
            ...
            trading_data_dir = Path(__file__).parent.parent.parent.parent / "trading-execution-engine" / "data"
    ...
    def _generate_sample_portfolio(self) -> Dict[str, Any]:
        ...
        for i, symbol in enumerate(all_symbols[:20]):  # Limit to 20 positions
            position_value = total_value * np.random.uniform(0.02, 0.08)  # 2-8% allocation
    ...
    async def _handle_risk_violations(self, violations: List[Dict[str, Any]]):
        ...
        for violation in violations:
            self.logger.error(f"RISK VIOLATION: {violation['rule']} - {violation['message']}")
    ...
    async def _handle_compliance_violations(self, violations: List[Dict[str, Any]]):
        ...
        for violation in violations:
            self.logger.error(f"COMPLIANCE VIOLATION: {violation['rule']} - {violation['message']}")
    ...
    async def _take_risk_action(self, violation: Dict[str, Any]):
        ...
        try:
            ...
            if action == 'reduce_position':
                self.logger.info(f"AUTO ACTION: Reducing position for {violation.get('symbol', 'portfolio')}")
                # Implement position reduction logic
                
            elif action == 'stop_trading':
                self.logger.critical("AUTO ACTION: Stopping all trading due to risk violation")
    ...
    async def _update_monitoring_dashboard(
        ...
        try:
            dashboard_data = {
                ...
                "risk_status": "violation" if (risk_violations or compliance_violations) else "normal"
            }
    ...
    async def generate_risk_report(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not portfolio_data:
                ...
                portfolio_data = {
                    "positions": [
                        {"symbol": "RELIANCE", "value": 500000, "quantity": 100, "pnl": 5000},
                        {"symbol": "TCS", "value": 300000, "quantity": 50, "pnl": 3000},
                        {"symbol": "INFY", "value": 200000, "quantity": 80, "pnl": -1000},
                        {"symbol": "HDFCBANK", "value": 400000, "quantity": 60, "pnl": 2000},
                        {"symbol": "ICICIBANK", "value": 150000, "quantity": 40, "pnl": 1500}
                    ]
                }
            ...
            total_pnl = sum(pos.get("pnl", 0) for pos in portfolio_positions if isinstance(pos, dict))
            ...
            report = {
                ...
                "portfolio_summary": {
                    ...
                    "leverage": total_value / max(total_value - abs(total_pnl), 1)  # Simplified leverage calc
                },
                "risk_metrics": risk_metrics,
            ...
            report_file = reports_dir / f"risk_report_{datetime.now().strftime('%Y-%m-%d')}.json"
    ...
    async def _generate_risk_recommendations(self, risk_metrics: Dict[str, Any]) -> List[str]:
        ...
        try:
            ...
            if abs(var_95) > 50000:  # > ₹50k VaR
                recommendations.append("Daily VaR exceeds ₹50,000. Consider reducing position sizes.")
            
            # Check concentration risk
            max_allocation = risk_metrics.get('max_single_allocation', 0)
            if max_allocation > 10:  # > 10% in single position
                recommendations.append(f"Single position allocation ({max_allocation:.1f}%) exceeds 10%. Diversify holdings.")
            ...
            for sector, allocation in sector_concentration.items():
                if allocation > 30:  # > 30% in single sector
                    recommendations.append(f"High concentration in {sector} sector ({allocation:.1f}%). Consider sector diversification.")
            
            # Check leverage
            leverage = risk_metrics.get('leverage', 1.0)
            if leverage > 2.0:
                recommendations.append(f"High leverage ({leverage:.2f}x) detected. Consider reducing leverage.")
            
            # Check correlation
            avg_correlation = risk_metrics.get('average_correlation', 0)
            if avg_correlation > 0.7:
                recommendations.append("High correlation between positions. Diversify with uncorrelated assets.")
...
async def main():
    ...
    parser.add_argument("--report", action="store_true", help="Generate risk report and exit")
    parser.add_argument("--test", action="store_true", help="Run test monitoring cycle and exit")
    ...
    if args.report:
        ...
        if "error" not in report:
            ...
            print(f"📊 Max Allocation: {risk_metrics.get('max_single_allocation', 0):.1f}%")
