...
class RiskManager:
    ...
    def __init__(self, 
        ...
        self.stress_test_scenarios = kwargs.get('stress_test_scenarios', ['2008_crisis', '2020_covid'])
        self.rebalance_threshold = kwargs.get('rebalance_threshold', 0.1)  # 10% deviation triggers rebalance
        
        logger.info(f"Risk Manager initialized with confidence level: {confidence_level*100}%, "
    ...
    def check_position_risk(self, symbol: str, position_size: float, 
        ...
        for pos in current_positions.values():
            if pos.get("sector") == symbol.split("_")[0]:  # Simple sector check based on prefix
        ...
        if action == "BUY" and symbol not in current_positions and total_positions >= max_positions:
            return RiskCheckResult(
                approved=False,
        ...
        return RiskCheckResult(
            ...
            risk_metrics={
                ...
                "sector_exposure": sector_exposure_pct if 'similar_exposure' in locals() else 0
            }
        )
        
    def check_trade_risk(self, symbol: str, quantity: int, price: float, 
                        action: str, portfolio_value: float, 
                        historical_data: Optional[pd.DataFrame] = None) -> RiskCheckResult:
        """
            ...
            if 'close' in historical_data.columns:
                ...
                if len(returns) > 30:  # Need sufficient data
                    ...
                    if var_pct > max_position_var_pct:
                        ...
                        adjusted_position_value = (max_position_var_pct / var_pct) * position_value
    ...
    def calculate_historical_var(self, 
                                ...
                                investment_value: float = 1.0) -> float:
        """
        ...
        if len(returns) < 30:
            logger.warning(f"Insufficient data for VaR calculation: {len(returns)} data points")
            return 0.0
    ...
    def calculate_parametric_var(self,
        ...
        if len(returns) < 30:
            logger.warning(f"Insufficient data for parametric VaR: {len(returns)} data points")
            return 0.0
    ...
    def calculate_cvar(self,
        ...
        if len(returns) < 30:
            logger.warning(f"Insufficient data for CVaR calculation: {len(returns)} data points")
            return 0.0
    ...
    def calculate_portfolio_var(self,
                               ...
                               method: str = 'historical') -> Tuple[float, Dict[str, float]]:
        """
        ...
        for symbol, pos in positions.items():
            if symbol in returns_data:
                if method == 'historical':
                    var = self.calculate_historical_var(returns_data[symbol], investment_value=pos['value'])
                else:
                    var = self.calculate_parametric_var(returns_data[symbol], investment_value=pos['value'])
                component_vars[symbol] = var
            else:
                logger.warning(f"No returns data for {symbol}, skipping in VaR calculation")
            ...
            if len(returns_df.columns) > 1:
                ...
                for i, symbol_i in enumerate(returns_df.columns):
                    for j, symbol_j in enumerate(returns_df.columns):
                        portfolio_variance += weights[symbol_i] * weights[symbol_j] * cov_matrix.iloc[i, j]
    ...
    def calculate_position_size(self,
        ...
        max_position_var = max_position_size = portfolio_value * self.max_position_size_pct
        ...
        var_95 = self.calculate_historical_var(returns, confidence_level=0.95, investment_value=actual_position_value)
        var_99 = self.calculate_historical_var(returns, confidence_level=0.99, investment_value=actual_position_value)
        cvar_95 = self.calculate_cvar(returns, confidence_level=0.95, investment_value=actual_position_value)
        cvar_99 = self.calculate_cvar(returns, confidence_level=0.99, investment_value=actual_position_value)
    ...
    def calculate_complete_risk_metrics(self, returns: pd.DataFrame) -> Dict[str, RiskMetrics]:
        """
        ...
        for symbol in returns.columns:
            ...
            if len(asset_returns) < 30:
                logger.warning(f"Insufficient data for {symbol}, skipping risk metrics calculation")
                continue
            ...
            if market_returns is not None:
                ...
                aligned_data = pd.DataFrame({'asset': asset_returns, 'market': market_returns})
