...
class RealTimeRiskMonitor:
    ...
    def check_portfolio_risk(
        ...
        try:
            ...
            if "error" in risk_metrics:
                self.logger.error(
                    f"Error calculating risk metrics: {risk_metrics['error']}"
                )
                return alerts
    ...
    def _check_limit(
        ...
        try:
            ...
            if is_breached:
                message = (
                    f"Risk limit '{limit.name}' breached: "
    ...
    def acknowledge_alert(self, alert_index: int):
        ...
        if 0 <= alert_index < len(self.active_alerts):
            ...
            self.logger.info(
                f"Acknowledged alert: {self.active_alerts[alert_index].limit_name}"
            )

    def clear_acknowledged_alerts(self):
        """Remove acknowledged alerts"""
        self.active_alerts = [
            alert for alert in self.active_alerts if not alert.acknowledged
        ]
        ...
        try:
            for limit_name, limit_config in config.items():
                ...
                if "last_breach" in limit_config and limit_config["last_breach"]:
                    limit.last_breach = datetime.fromisoformat(
                        limit_config["last_breach"]
                    )
